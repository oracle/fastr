...
public class TestrGenBuiltinisfunction extends TestBase {
	...
	public void testisfunction27() {
		assertEval(Ignored.Unknown, "argv <- list(function(name, cond = NULL) {    i <- 1L    repeat {        r <- .Internal(.getRestart(i))        if (is.null(r)) return(NULL) else if (name == r[[1L]] &&             (is.null(cond) || is.null(r$test) || r$test(cond))) return(r) else i <- i +             1L    }});"+
			"do.call(\'is.function\', argv)");
	}
	...
	public void testisfunction29() {
		assertEval("argv <- list(function(x, i, ...) structure(NextMethod(\'[\'), class = class(x)));"+
			"do.call(\'is.function\', argv)");
	}
	...
	public void testisfunction32() {
		assertEval("argv <- list(function(cpu = Inf, elapsed = Inf, transient = FALSE) .Internal(setTimeLimit(cpu,     elapsed, transient)));"+
			"do.call(\'is.function\', argv)");
	}
