...
public class TestrGenBuiltindfltWarn extends TestBase {
    ...
    public void testdfltWarn3() {
        assertEval(Ignored.Unknown, "argv <- list(\'glm.fit: algorithm stopped at boundary value\', NULL); .Internal(.dfltWarn(argv[[1]], argv[[2]]))");
    }

    @Test
    public void testdfltWarn4() {
        assertEval(Ignored.Unknown, "argv <- list(\'header and 'col.names' are of different lengths\', quote(read.table(\'foo3\', header = TRUE, col.names = letters[1:4]))); .Internal(.dfltWarn(argv[[1]], argv[[2]]))");
    }

    @Test
    public void testdfltWarn5() {
        assertEval(Ignored.Unknown, "argv <- list(\'‘graphics’ namespace cannot be unloaded:\\n  namespace ‘graphics’ is imported by ‘stats’ so cannot be unloaded\', NULL); .Internal(.dfltWarn(argv[[1]], argv[[2]]))");
    }
    ...
    public void testdfltWarn8() {
        assertEval(Ignored.Unknown, "argv <- list(\'prediction from a rank-deficient fit may be misleading\', quote(predict.lm(object, newdata, se.fit, scale = residual.scale, type = ifelse(type == \'link\', \'response\', type), terms = terms, na.action = na.action))); .Internal(.dfltWarn(argv[[1]], argv[[2]]))");
    }

    @Test
    public void testdfltWarn9() {
        assertEval(Ignored.Unknown, "argv <- list(\'1 y value <= 0 omitted from logarithmic plot\', quote(xy.coords(x, NULL, log = log))); .Internal(.dfltWarn(argv[[1]], argv[[2]]))");
    }
