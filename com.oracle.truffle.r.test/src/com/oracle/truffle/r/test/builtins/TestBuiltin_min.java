...
public class TestBuiltin_min extends TestBase {
    ...
    public void testmin5() {
        assertEval(Ignored.Unknown, "argv <- list(structure(list(c(1L, 2L, 4L), 1:3, c(2L, 1L)), class = c('package_version', 'numeric_version')), na.rm = FALSE);min(argv[[1]],argv[[2]]);");
    }
    ...
    public void testmin14() {
        assertEval(Ignored.Unknown, "argv <- list(structure(c(13823, NA), class = 'Date'), na.rm = TRUE);min(argv[[1]],argv[[2]]);");
    }

    @Test
    public void testmin15() {
        assertEval(Ignored.Unknown, "argv <- list(structure(c(1208822400, 1209168000), class = c('POSIXct', 'POSIXt'), tzone = 'GMT'), na.rm = FALSE);min(argv[[1]],argv[[2]]);");
    }
