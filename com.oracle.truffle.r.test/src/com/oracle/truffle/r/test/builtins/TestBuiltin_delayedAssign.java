...
public class TestBuiltin_delayedAssign extends TestBase {
    ...
    public void testDelayedAssign() {
        ...
        assertEval(Output.ContainsError, "{ f <- function() { delayedAssign(\"x\",y); delayedAssign(\"y\",x); list(x, y)}; f() }");
        assertEval(Output.ContainsError, "{ f <- function() { delayedAssign(\"x\",y); delayedAssign(\"y\",x); paste(x, y)}; f() }");
        assertEval(Output.ContainsError, "{ f <- function() { delayedAssign(\"x\",y); delayedAssign(\"y\",x); print(x, y)}; f() }");
