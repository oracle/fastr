...
public class TestSimpleVectors extends TestBase {
    ...
    public void testMoreVectorsOther() {
        ...
        assertEval("{ x <- integer() ; x[[NA]] <- NULL ; x }");
        assertEval(Ignored.Unknown, Output.ContainsError, "{ x <- c(1) ; x[[NA]] <- NULL ; x }");
        assertEval("{ x <- c(1,2) ; x[[NA]] <- NULL ; x }");
        assertEval("{ x <- c(1,2,3) ; x[[NA]] <- NULL ; x }");
    ...
    public void testIgnored1() {
        Assert.assertEquals(TEST_IGNORED1_EXPECTED_VAL, fastREval("{ x <- c(a=as.raw(10), b=as.raw(11), c=as.raw(12)) ; x[10] }"));
    }

    @Test
    public void testIgnored2() {
        Assert.assertEquals(TEST_IGNORED2_EXPECTED_VAL, fastREval("{ x <- c(a=as.raw(10),b=as.raw(11),c=as.raw(12),d=as.raw(13)) ; f <- function(s) { x[s] } ; f(TRUE) ; f(1L) ; f(as.character(NA)) }"));
    }

    @Test
    public void testIgnored3() {
        String s = fastREval("{ x <- c(a=as.raw(10),b=as.raw(11),c=as.raw(12),d=as.raw(13)) ; f <- function(s) { x[c(s,s)] } ; f(TRUE) ; f(1L) ; f(as.character(NA)) }");
        Assert.assertEquals(TEST_IGNORED3_EXPECTED_VAL, s);
    }
