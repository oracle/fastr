 ...
 * last time.
 */
public abstract class CopyAttributesNode extends Node {
    ...
    protected static boolean isRVector(RAbstractVector vector) {
    ...
    public RAbstractVector copySameLength(RAbstractVector target, RAbstractVector left, @SuppressWarnings("unused") int leftLength, RAbstractVector right, @SuppressWarnings("unused") int rightLength, //
        ...
        if (copyAllAttributes) {
            if (result != right) {
                copyOfRegLeft.execute(right, result);
            }
            if (result != left) {
                copyOfRegRight.execute(left, result);
            }
        }
            ...
            if (newDimensions == null) {
                ...
                if (result != left) {
                    ...
                    if (hasNamesLeft.profile(vecNames != null)) {
                        result.initAttributes().put(RRuntime.NAMES_ATTR_KEY, vecNames);
