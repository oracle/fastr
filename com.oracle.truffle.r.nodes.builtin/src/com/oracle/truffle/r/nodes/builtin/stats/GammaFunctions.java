...
public abstract class GammaFunctions {
    ...
    public abstract static class DiGamma extends RBuiltinNode {
        ...
        protected RDoubleVector digamma(RAbstractIntVector x) {
            return digamma(RClosures.createIntToDoubleVector(x, naClosureCheck));
        }

        @Specialization
        protected RDoubleVector digamma(RAbstractLogicalVector x) {
            return digamma(RClosures.createLogicalToDoubleVector(x, naClosureCheck));
        }
    ...
    @CompilationFinal private static final double[] bvalues = new double[]{1.00000000000000000e+00, -5.00000000000000000e-01, 1.66666666666666667e-01, -3.33333333333333333e-02,
                    ...
                    4.29614643061166667e+11, -1.37116552050883328e+13, 4.88332318973593167e+14, -1.92965793419400681e+16};

    private static final int n_max = 100;
    // the following is actually a parameter in the original code, but it's always 1 and must be as
    // the original code treats the "ans" value of type double as an array, which is legal only if a
    // the first element of the array is accessed at all times
...
// ans[k] = ans[k-1] / x;
                        assert mm < 2;
                        // int the original code, ans should not be accessed beyond the 0th index
                    }
                    if (n == 0 && kode == 2) {
// ans[0] += xln;
                        mVal.ans += xln;
