...
# finesse missing .Internals
#   existsInFrame <- function (x, env) .Internal(exists(x, env, "any", FALSE))
    existsInFrame <- function (x, env) exists(x, where=env, mode="any", inherits=FALSE)
#   getFromFrame <- function (x, env) get(x, env, "any", FALSE)
    getFromFrame <- function (x, env) .Internal(get(x, pos=env, mode="any", inherits=FALSE))
#   set <- function (x, value, env) .Internal(assign(x, value, env, FALSE))
    set <- function (x, value, env) assign(x, value, pos=env, inherits=FALSE)
...
lazyLoad <- function(filebase, envir = parent.frame(), filter)
{
    fun <- function(db) {
        ...
        # debug.break(list(vars, vals, expr, db, envir))
