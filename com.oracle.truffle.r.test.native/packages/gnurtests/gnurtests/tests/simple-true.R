...
typeof(1e3L) == "integer"
# FastR commented-out(extra whitespace in message): typeof(1e-3L) == "double" # gives warning
# FastR commented-out(extra whitespace in message): 1.L # gives warning
# FastR commented-out(extra output from parser): try(parse(text = "12iL")) # gives syntax error
...
## crossing bug PR#40
# FastR commented-out(FALSE vs TRUE and warnings): is.factor(ff <- gl(2,3) : gl(3,2)) && length(ff) == 6
# FastR commented-out: all(levels(ff) == t(outer(1:2, 1:3, paste, sep=":")))
## from PR#5
...
f1 <- y1 ~ x1
f2 <- y2 ~ x2
# FastR commented-out(FALSE vs TRUE and warning): f2[2] <- f1[2]
# FastR commented-out(FALSE vs TRUE): deparse(f2) == "y1 ~ x2"
