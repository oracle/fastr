...
# FastR changed(time difference printed: 0 vs 0.01):     print(ct3 <- system.time(N <- rhyper(100, 8000, 1e9-8000, 1e6))[1]) < 0.02
ct3 <- system.time(N <- rhyper(100, 8000, 1e9-8000, 1e6))[1] < 0.2
    ,
...
p <- m/N; q <- 1 - p
# FastR commented-out(%g printing extra zeros): cat(sprintf(
# FastR commented-out:    "N = n+m = %g, m = Np = %g; k = %g ==> (p,f) = (m,k)/N = (%g, %g)\n k*p*q = %.4g > 1: %s\n",
# FastR commented-out:     N, m, k, m/N, k/N, k*p*q, k*p*q > 1))
set.seed(11)
