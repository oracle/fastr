...
# FastR changed(results differ): (txp <- tx[tx >= 1])#-- Positive exponent -- 4 values
txp <- tx[tx >= 1] #-- Positive exponent -- 4 values
# FastR changed(results differ): (txn <- tx[tx <	 1])#-- Negative exponent -- 7 values
...
x0 <- x <- c(1+1i, 1.2 + 10i)
names(x) <- c("a","b")
# FastR commented-out: x
(xx <-	rbind(x,  2*x))
	rbind(x0, 2*x0)
x[4:6] <- c(Inf,Inf*c(-1,1i))
# FastR commented-out: x  + pi
matrix(x + pi, 2)
matrix(x + 1i*pi, 3)
xx + pi
...
sf(2)
sf(3.141)
# FastR commented-out: sf(-1.25, 20)
...
# FastR commented-out: e1 <- tryCatch(options(max.print=Inf), error=function(e)e)
e2 <- tryCatch(options(max.print= 0),  error=function(e)e)
# FastR commented-out: stopifnot(inherits(e1, "error"))
