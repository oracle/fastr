...
public class RRawNativeVectorData implements TruffleObject, ShareableVectorData {
    ...
    private final RRawVector vec;

    public RRawNativeVectorData(RRawVector vec) {
        this.vec = vec;
    }
    ...
    @ExportMessage
    public RRawNativeVectorData materialize() {
        return this;
    }
    ...
    public RRawArrayVectorData copy(@SuppressWarnings("unused") boolean deep) {
        byte[] data = NativeDataAccess.copyByteNativeData(vec.getNativeMirror());
        return new RRawArrayVectorData(data);
    }

    @ExportMessage
    public byte[] getReadonlyRawData() {
        return getRawDataCopy();
    }
    ...
    @ExportMessage
    public byte getRaw(@SuppressWarnings("unused") RandomAccessIterator it, int index,
                    @Shared("naCheck") @Cached() NACheck naCheck) {
        byte value = NativeDataAccess.getData(vec, null, index);
        naCheck.check(value);
        return value;
    ...
    @ExportMessage
    public void setRaw(@SuppressWarnings("unused") RandomAccessWriteIterator it, int index, byte value) {
        NativeDataAccess.setData(vec, null, index, value);
    }
}
